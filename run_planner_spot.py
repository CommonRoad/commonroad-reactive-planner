...
# *************************************
spot_manager = SpotManager(spot_update_dict)
...
while not planner.goal_reached():
    ...
    if plan_new_trajectory:
        ...
        spot_manager.reset(config, current_count)
        spot_manager.predict()

        # update planner collision checker with scenario containing SPOT predicitions
        planner.set_collision_checker(spot_manager.scenario_spot)
        ...
        # reset planner initial state for re-planning
        ...
        # visualization only: create ego Vehicle for planned trajectory and store sampled trajectory set
    ...
    if config.debug.show_plots or config.debug.save_plots:
        visualize_planner_at_timestep(scenario=spot_manager.scenario_spot, planning_problem=config.planning_problem,
