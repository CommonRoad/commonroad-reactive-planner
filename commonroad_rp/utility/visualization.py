...
def visualize_planner_at_timestep(
    ...
    :param medium_goal: medium goal visualization
    ...
    rnd.draw_params.dynamic_obstacle.trajectory.draw_trajectory = config.debug.draw_prediction_others
    ...
    if medium_goal is not None:
        rect = [patches.Rectangle((medium_goal[0] - 0.5, medium_goal[1] - 0.5), 1, 1, linewidth=1, edgecolor='g',
                                  facecolor='g')]
        coll = PatchCollection(rect, zorder=10)
        rnd.ax.add_collection(coll)
    ...
    if traj_set is not None:
        for i in range(0, len(traj_set), step):
            if traj_set[i].feasibility_label.name in config.debug.traj_vis_filter:
                continue
            color = config.debug.traj_color[traj_set[i].feasibility_label.name]
...
def visualize(task):
    ...
    if task.plot_limits is None and task.ref_path is not None:
         ...
         task.plot_limits = [x_min, x_max, y_min, y_max]
    ...
    if task.medium_goal is not None:
        rect = [
            patches.Rectangle((task.medium_goal[0] - 0.5, task.medium_goal[1] - 0.5), 1, 1, linewidth=1, edgecolor='g',
                              facecolor='g')]
        coll = PatchCollection(rect, zorder=10)
        task.rnd.ax.add_collection(coll)
    ...
    if task.traj_set is not None:
        for i in range(0, len(task.traj_set), step):
            if task.traj_set[i].feasibility_label.name in task.traj_vis_filter:
                continue
            color = task.traj_color[task.traj_set[i].feasibility_label.name]
    ...
    if task.save_plots:
        ...
        plt.savefig(f"{plot_dir}/{task.scenario.scenario_id}_{task.timestep}.{task.plots_file_format}",
                    format=task.plots_file_format, dpi=300, bbox_inches='tight')
...
class VisualizationTask:
    ...
    def __init__(self, scenario: Scenario, planning_problem: PlanningProblem, ego: DynamicObstacle,
        ...
        self.plots_file_format = config.debug.plots_file_format
        self.traj_vis_filter = config.debug.traj_vis_filter
        self.traj_color = config.debug.traj_color
